...
class buy:
    def __init__(self, parent):
        ...
        class dropdown:
            # dropdown menu (row 2)
            items_label = ttk.Label(text="Items for sale")
            items_label.grid(row=2, column=0)

            items_list = ["Igor - Tyler The Creator"
                 "Good Kid Maad City - Kendrick Lamar"
                 "Demon Days - Gorillaz"]
            items_action = StringVar()
            items_action.set(items_list[0])

            items_box = ttk.Combobox(textvariable = items_action, state="readonly")
            items_box['values'] = items_list
            items_box.grid(row=2, column=1, padx=10, pady=3, sticky="WE")
